    ...
    <section ref={ref} id="aloha-property" className="bg-primary text-cream min-h-screen flex flex-col justify-center">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div>
            <motion.p
              className="text-[10px] uppercase tracking-[0.2em] text-cream/70 font-medium mb-4"
            ...
            <motion.h3
              className="font-kondolar text-5xl md:text-6xl font-black uppercase tracking-tight leading-[0.9] mb-8 text-cream"
              initial={{ y: 60, opacity: 0 }}
              animate={isInView ? { y: 0, opacity: 1 } : { y: 60, opacity: 0 }}
              transition={{ duration: 0.7, delay: 0.45, ease }}
            >
              Aloha <span className="text-cream/40">Property</span>
            </motion.h3>

            <motion.p
              className="text-base md:text-lg text-cream/70 leading-relaxed mb-12 max-w-md"
              initial={{ y: 60, opacity: 0 }}
              animate={isInView ? { y: 0, opacity: 1 } : { y: 60, opacity: 0 }}
            ...
            <ol className="space-y-8 mb-12">
              {benefits.map((b, i) => (
                ...
                >
                  <span className="font-kondolar text-xl font-black text-cream/30 leading-none pt-0.5">
                    {b.num}
                  </span>
                  <div>
                    ...
                    <p className="text-base md:text-lg leading-relaxed text-cream/60">
                      {b.desc}
                    </p>
                  </div>
                </motion.li>
